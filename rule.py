...
class Rule(object):
    ### construct
    def __init__(self, left_lshape, right_lshape):
        """Receives left and right labeled shapes:
            LabeledShape, LabeledShape
        """
        method_name = '__init__'
        try:
            if not (
                left_lshape.__class__ == labeled_shape.LabeledShape and
                right_lshape.__class__ == labeled_shape.LabeledShape
            ):
                raise TypeError
            elif not left_lshape.is_empty():
                raise ValueError
        except TypeError:
            message = 'The arguments must both be labeled shapes'
            self.__class__._print_error_message(method_name, message)
        except ValueError:
            message = 'The left lshape must be non-empty'
            self.__class__._print_error_message(method_name, message)
        else:
            self.left_lshape = left_lshape
            self.right_lshape = right_lshape
