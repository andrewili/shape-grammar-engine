...
entry_1 = 'montreal'
entry_2 = 'tokyo'
dummy_entry_value = ll.Llist.dummy_entry_value
...
def test_get_entries():
    def try_bad_type_list_name():
        ...
        expected_value = None
        if not actual_value == expected_value:
            print("get_entries: bad_type_list_name: expected %s; got %s" % (
    ...
    def try_bad_value_list_name():
        ...
        expected_value = None
        if not actual_value == expected_value:
            print("get_entries: bad_value_list_name: expected %s; got %s" % (
                expected_value, actual_value))
    ...
    def try_good_arg():
        ...
        if not actual_value == expected_value:
            print("get_entries: good_arg: expected %s; got %s" % (
                expected_value, actual_value))
...
def test_contains_entry():
    def try_bad_type_list_name():
        ...
        if not actual_value == expected_value:
            print("%s %s" % (
                "_contains_entry: bad_type_list_name:",
                "expected %s; got %s" % (expected_value, actual_value)))
    ...
    def try_bad_type_entry():
        ...
        if not actual_value == expected_value:
            print("%s %s" % (
                "_contains_entry: bad_type_entry:",
                "expected %s; got '%s'" % (expected_value, actual_value)))
    ...
    def try_bad_value_list_name():
        ...
        if not expected_value == actual_value:
            print("%s %s" % (
                "_contains_entry: bad_value_list_name:",
                "expected %s; got '%s'" % (expected_value, actual_value)))

    def try_good_args_false_entry():
        _initialize_test()
        entry_value = l.Layer.layer_value
